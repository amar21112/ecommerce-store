...
class Template
{
  ...
  {
    $this->_action_views = $actionViewPath;
  }
  ...
  {
    extract($this->_data); //Import variables into the current symbol table from an array
    ...
    if(!array_key_exists('template', $this->_templateParts)) {
      ...
      if(!empty($parts)) {
        ...
        foreach ($parts as $partKey => $file) {
          if($partKey === ':view') {
            require_once $this->_action_views;
          } else {
            require_once $file;
          }
        }
